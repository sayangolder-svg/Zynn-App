...
export default function HomeScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      api.get<Array<Record<string, unknown>>>("/reels/").then((data) => {
        const next: ShoppableReel[] = data.map((item) => ({
            ...
            const resolvedDescription =
              rawDescription
                || (productNames.length > 1
                  ...
                  : (productNames[0] ? `Detected product: ${productNames[0]}` : ""));
      ...
      api.get<EarningsSummary>("/earnings/summary/").then(setSummary).catch(() => {});
    }, []),
  );
    ...
    <SafeAreaView className="flex-1 bg-black pt-5" edges={["top", "left", "right"]}>
      ...
      >
        ...
        <View className="flex-row px-5 gap-3 mb-8">
          <StatCard icon="trending-up" label="Earnings" value={`Rs ${summary?.total_earnings ?? "0.00"}`} />
          <StatCard icon="cart-outline" label="Orders" value={String(reels.length)} />
          <StatCard icon="share-outline" label="Shares" value={String(reels.reduce((a, r) => a + r.shares, 0))} />
        </View>
