...
export default function AddReelScreen() {
  ...
  const pollCartForUpdates = async (cid: string) => {
    try {
      console.log(`🔄 [reels-tab] Polling cart: ${cid}`);
  ...
  const fetchReelData = async (url: string) => {
    ...
    try {
      ...
      console.log("🔌 [reels-tab] Reel Response Received:");
      ...
      if (hasPricesPending && responseCartId) {
        console.log(`🔄 [reels-tab] Starting polling for cart: ${responseCartId}`);
        if (pollIntervalRef.current) {
          clearInterval(pollIntervalRef.current);
        }
        pollCartForUpdates(responseCartId as string);
        pollIntervalRef.current = setInterval(() => {
          pollCartForUpdates(responseCartId as string);
        }, 3000);
      } else {
        console.log(`⚠️ [reels-tab] Polling skipped - hasPricesPending=${hasPricesPending}, cartId=${responseCartId}`);
      }
